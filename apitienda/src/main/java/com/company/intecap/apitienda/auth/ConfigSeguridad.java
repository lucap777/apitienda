...
public class ConfigSeguridad extends WebSecurityConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests().antMatchers(GET, "/api/v1/fabricantes").permitAll() //permitAll: permite a todos los usuarios acceder a la ruta
                //para todos lo demas se requiere autenticacion
               // .antMatchers(POST, "/api/v1/fabricantes").permitAll()
    ...
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/v2/api-docs", "/configuration/ui", "/swagger-resources/**",
                "/configuration/**","/swagger-ui.html","/webjars/**","/api/v1/usuarios");
    }
}
